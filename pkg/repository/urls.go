...
type URLRepositorier interface {
	...
	InsertURLs(ctx context.Context, urls []string) error
}
...
func (u *urlRepository) InsertURLs(ctx context.Context, urls []string) error {
	_, err := u.postgreQueries.InsertURLs(ctx, urls)
	return err
}
